...
if not os.path.exists(ProcessedPath):
    os.makedirs(ProcessedPath)  #os.makedirs用于创建目录
imagelist = os.listdir(ImgPath)
#os.listdir用于返回指定文件或者文件夹的名字
for image in imagelist:
    print('a new image:', image)
    image_pre, ext = os.path.splitext(image)
    #os.path.splitext用于分离文件名和扩展名
    imgfile = ImgPath + image
    xmlfile = AnnoPath + image_pre + '.xml'
    DomTree = xml.dom.minidom.parse(xmlfile)#打开xml文档
    annotation = DomTree.documentElement#得到文档元素对象
    filenamelist = annotation.getElementsByTagName('filename') #[<DOM Element: filename at 0x381f788>]
    filename = filenamelist[0].childNodes[0].data#获得标签的数据
